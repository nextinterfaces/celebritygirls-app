...
import next.celebs.dao.WikipediaDao;
	...
	private void listenEvents() {
		...
		ctx.getEventBus().addHandler(RedcarpetOptionEvent.TYPE, new RedcarpetOptionHandler() {
			...
			public void onExecute(RedcarpetOptionEvent e) {
				if (e.getType() == next.celebs.event.RedcarpetOptionEvent.Type.OSCARS) {
					setEventType(Type.REDCARPET_OSCARS);

				} else if (e.getType() == next.celebs.event.RedcarpetOptionEvent.Type.GOLDENGLOBE) {
					setEventType(Type.REDCARPET_GOLDEN);

				} else if (e.getType() == next.celebs.event.RedcarpetOptionEvent.Type.GRAMMY) {
					setEventType(Type.REDCARPET_GRAMMY);

				} else if (e.getType() == next.celebs.event.RedcarpetOptionEvent.Type.CANNES) {
			...
			if (et == Type.REDCARPET_OSCARS || et == Type.REDCARPET_GOLDEN || et == Type.REDCARPET_GRAMMY
				...
				wikiBtn.addMouseDownHandler(new MouseDownHandler() {
					...
					public void onMouseDown(MouseDownEvent event) {
						final WikiPopup popup = new WikiPopup(celebName);
						popup.doShow();
						WikipediaDao.init(celebName, ctx, new Response<String[]>() {
							@Override
